...
> 📌 **字体排版全生态唯一事实源**：详见 **`docs/11-typography-and-font-spec.md`**。所有界面文本必须引用 `@dimen/nokia_font_*` 6 级语义 Token（`display:16sp`, `title:13sp`, `body:12sp`, `small_title:11sp`, `caption:9sp`, `micro:7sp`），严禁在布局 XML 中硬编码裸数字号或滥用加粗。
