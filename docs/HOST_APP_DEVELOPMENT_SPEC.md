# KeydroidX 宿主应用诺基亚风格开发与接入规范指南

> 本文档是面向生态独立应用（宿主 App）的**强制开发规范指南**。
> 接入 KeydroidX 生态的独立应用，必须严格遵守本文档中定义的类继承体系、UI 渲染准则、按键交互规范与弹窗契约，以确保在按键机上呈现纯正、一致的诺基亚复古风格体验，彻底杜绝 Android 原生触屏风格残留。

---

## 目录

1. [核心继承体系与架构契约](#一核心继承体系与架构契约)
   - 1.1 [窗口骨架：NokiaBaseActivity](#11-窗口骨架-nokiabaseactivity)
   - 1.2 [页面基类：NokiaPageFragment 三大模板](#12-页面基类-nokiapagefragment-三大模板)
   - 1.3 [页面描述契约：NokiaPage 接口](#13-页面描述契约-nokiapage-接口)
2. [视觉与 UI 规范（强制规则）](#二视觉与-ui-规范强制规则)
   - 2.1 [字体规范：点阵字体全局渲染](#21-字体规范点阵字体全局渲染)
   - 2.2 [配色规范：动态生态主题跟随](#22-配色规范动态生态主题跟随)
   - 2.3 [图标规范：统一 MaterialIcons 字符图标](#23-图标规范统一-materialicons-字符图标)
   - 2.4 [软键栏规范：静态标签与 INVISIBLE 占位](#24-软键栏规范静态标签与-invisible-占位)
3. [物理按键与交互分发规范（灵魂法则）](#三物理按键与交互分发规范灵魂法则)
   - 3.1 [按键语义转换：禁止写死 KeyCode](#31-按键语义转换禁止写死-keycode)
   - 3.2 [DOWN / UP 完整配对：防点击合成误触](#32-down--up-完整配对防点击合成误触)
   - 3.3 [首键防吞规范：Touch Mode 焦点陷阱](#33-首键防吞规范touch-mode-焦点陷阱)
   - 3.4 [列表循环导航与平滑滚动算法](#34-列表循环导航与平滑滚动算法)
4. [标准复古弹窗体系](#四标准复古弹窗体系)
5. [跨进程通信与 AndroidManifest 配置](#五跨进程通信与-androidmanifest-配置)
6. [开发自查清单与速记口诀](#六开发自查清单与速记口诀)
