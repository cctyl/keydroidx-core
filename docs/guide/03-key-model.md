...
物理按键与语义动作的双向映射。核心思想：**业务代码只认 `KeydroidxKeyAction` 语义常量，永远不写死 keyCode**；同一套代码在任意按键机上，只要用户（或桌面）配过键即可工作。

## KeydroidxKeyAction

`io.github.cctyl.nokia.common.model.KeydroidxKeyAction`

不可实例化的常量类。

请用 `io.github.cctyl.nokia.common.model.KeydroidxKeyAction`；`keycore.model.KeydroidxKeyAction` 这个同名桥接类已于 2026-09-06 删除（原为 10 行空壳）。
...
`io.github.cctyl.nokia.keycore.model.KeydroidxKeyBinding`
...
```java
public KeydroidxKeyBinding()                       // 初始化为标准兜底映射
public KeydroidxKeyBinding(@NonNull Context context) // 初始化后立即 loadLocal(context)
```
...
```
从 KeyEvent 的 `getKeyCode()` 解析语义动作。**未绑定时按标准 Android 键码无条件兜底**（不依赖映射表是否有该条目）：

| 物理 KeyCode | 兜底动作 |
|:---|:---|
| `ENTER` / `NUMPAD_ENTER` / `DPAD_CENTER` | `SELECT` |
| `MENU` / `F1`（未绑定时） | `SOFT_LEFT` |
| `BACK` / `ESCAPE`（未绑定时） | `SOFT_RIGHT` |
| 其余查表，查不到返回 `UNKNOWN(-1)` |

> `keycore.model.KeydroidxKeyBinding` 是 key-core 的**真实实现**（非桥接）；桌面 launcher 侧另有一份独立实现，两端仅协议字段保持一致。
...
// 有本地配置才读入并返回 true；从未配过返回 false —— KeydroidxClient 四级降级的判断依据
...
public synchronized KeydroidxKeyBinding clone()         // 深拷贝（监听回调派发副本，防止外部篡改内部表）
...
override fun dispatchKeyEvent(event: KeyEvent): Boolean {
    if (event.action == KeyEvent.ACTION_DOWN && event.repeatCount == 0) {
        val action = KeydroidxClient.get(this).keyBinding.resolveAction(event)
...
```kotlin
val draft = KeydroidxClient.get(this).keyBinding.clone()
draft.bind(KeydroidxKeyAction.UP, event.keyCode)   // 用户按下某物理键
draft.save(this)                                // 写本地
KeydroidxClient.get(this).reload()                  // 立即生效并通知全局
```

## 相关文档

- 谁来调用 resolveAction 并分发 → [04-base-activity](./04-base-activity.md)
- 录入向导 →  [10-key-wizard](./10-key-wizard.md)
