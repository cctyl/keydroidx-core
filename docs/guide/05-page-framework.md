...
主包：`io.github.cctyl.nokia.common.ui.page`

> `KeydroidxFocusHost` 不在本包，位于 `io.github.cctyl.nokia.common.ui.focus`。
...
```
                    ┌───────────────────────────┐
                    │     KeydroidxFocusHost     │ (按键事件分发契约: UP/DOWN/LEFT/RIGHT/SELECT/SOFT...)
                    └──────────────┬────────────┘
                                   │ extends
                    ┌──────────────┴────────────┐
                    │        KeydroidxPage       │ (声明式页面契约: getPageTitle / getSoft*Text)
                    └──────────────┬────────────┘
                                   │ implements
                    ┌──────────────┴────────────┐
                    │    KeydroidxPageFragment   │ (Fragment 根基类: 字体/主题/可视区滚动/契约同步)
                    └──────┬─────────────┬─────┘
                           │             │
            ┌──────────────┴──┐   ┌─────┴─────────────────────┐
            │ KeydroidxList   │   │ KeydroidxScrollPageFragment│
            │ PageFragment    │   │ (长文/表单自动平滑步进滚动) │
            └─────────────────┘   └───────────────────────────┘
                    │
                    ▼ uses
       ┌────────────────────────────┐
       │   KeydroidxListFocusHelper │ ◄── [独立复用] Activity / ViewPager / 自定义 View 亦可直接使用
       │  (循环导航/高亮/自动可视滚动)│
       └────────────────────────────┘
```

要点：`KeydroidxListFocusHelper` 位于继承体系**之外**，是组合式组件——不用 Fragment 的 Activity 页面可直接持有一个实例，见 [06-list-focus](./06-list-focus.md)。

## 接口层

### KeydroidxFocusHost — 按键事件接收
...
boolean onBack()                    // 返回键
```
返回 `true` 表示消费该事件。由宿主（`KeydroidxBaseActivity.onAction`）调用。

### KeydroidxPage — 标题与软键声明（extends KeydroidxFocusHost）
...
### KeydroidxPageHost — 宿主侧契约

由 `KeydroidxBaseActivity` 实现，方法说明见 [04-base-activity](./04-base-activity.md)。

## 基类层

### KeydroidxPageFragment — 普通功能页基类
...
// 生命周期安全的滚动跟随（isAdded 守护），委托 KeydroidxListFocusHelper 静态实现
```
...
| `onBack()` | 有 KeydroidxPageHost 则 `exitCurrent()`，否则 `activity.onBackPressed()` |

### KeydroidxListPageFragment — 单列列表页基类
...
protected void applyFocusBackground()   // 绘制 KeydroidxTheme.createSelectionDrawable 高亮并持焦
...
### KeydroidxScrollPageFragment — 长内容滚动页基类
...
| 页面形态 | 基类 |
|----------|------|
| 单列菜单 / 列表 | `KeydroidxListPageFragment` |
| 文章 / 说明 / 长表单 | `KeydroidxScrollPageFragment` |
| 自由布局（播放器、宫格） | `KeydroidxPageFragment` 自己管焦点 |
| 不用 Fragment 的 Activity 页 | 直接继承 `KeydroidxBaseActivity` + 组合 `KeydroidxListFocusHelper` |
