# 06 · 列表焦点控制（KeydroidxListFocusHelper）

`io.github.cctyl.nokia.common.ui.page.KeydroidxListFocusHelper`
...
> 继承 `KeydroidxListPageFragment` 时无需此类（基类内置了同款逻辑）；两者不要叠加使用。

## 构造

```java
public KeydroidxListFocusHelper()                                    // 之后 setScrollView()
public KeydroidxListFocusHelper(@Nullable ScrollView scrollView)     // 构造时绑定滚动容器
public KeydroidxListFocusHelper(@Nullable Context context, @Nullable ScrollView scrollView)
...
```java
public KeydroidxListFocusHelper setScrollView(@Nullable ScrollView scrollView)  // 绑定滚动容器
public KeydroidxListFocusHelper setCyclic(boolean cyclic)   // 是否首尾循环，默认 true
...
`KeydroidxPageFragment.smoothScrollToVisible` 与 `KeydroidxBaseActivity` 同名方法均为其变体——**全生态统一走这条路径，禁止手写 `getTop()` 定位滚动**。

## 典型用法（Activity 页面）

```kotlin
private lateinit var focus: KeydroidxListFocusHelper

override fun onInitViews() {
    val scroll = findViewById<ScrollView>(R.id.scroll)
    focus = KeydroidxListFocusHelper(scroll)
    focus.setItems(listOf(row1, row2, row3))
    focus.setFocusIndex(0)
}

override fun onAction(action: Int): Boolean = when (action) {
    KeydroidxKeyAction.UP, KeydroidxKeyAction.DOWN -> focus.onDirection(action)
    KeydroidxKeyAction.SELECT -> onSelect(focus.focusIndex); true
...
- 高亮色来源 → [08-theme-font-icons](./09-theme-font-icons.md)（KeydroidxTheme.createSelectionDrawable）
