...
包：`io.github.cctyl.nokia.common.ui.dialog`（`keycore.ui.dialog` 下同名桥接类已于 2026-09-06 删除）
...
```java
public KeydroidxOptionsDialog(@NonNull Context context)                    // 标题默认 "选项"
...
```
`id` 用于不依赖下标的稳定判断；图标建议用 `KeydroidxIcons.createDrawable(...)` 生成。

### 链式装配

```java
public KeydroidxOptionsDialog addItem(CharSequence title)                      // 无图标
public KeydroidxOptionsDialog addItem(int id, CharSequence title)              // 带 id
public KeydroidxOptionsDialog addItem(int id, CharSequence title, Drawable icon) // 带 id+图标
public KeydroidxOptionsDialog setOnOptionSelectedListener(OnOptionSelectedListener listener)
...
```kotlin
KeydroidxOptionsDialog(this, "歌曲操作")
    .addItem(1, "播放", KeydroidxIcons.createDrawable(this, KeydroidxIcons.ICON_PLAY, size, Color.WHITE))
...
## KeydroidxConfirmDialog — 确认 / 提示

```java
public KeydroidxConfirmDialog(@NonNull Context context, @NonNull String title, @NonNull String message)

public KeydroidxConfirmDialog setPositiveButton(String text, OnConfirmListener listener)
public KeydroidxConfirmDialog setNegativeButton(String text, @Nullable Runnable listener)
// 默认按钮文案：确认 / 取消
...
```kotlin
KeydroidxConfirmDialog(this, "删除歌单", "确定删除「${name}」吗？")
...
public KeydroidxTextInputFragment setOnConfirmListener(OnConfirmListener listener)
public KeydroidxTextInputFragment setRequired(boolean required)   // 默认 true，空内容时提示
...
## KeydroidxDialogFocus — 弹窗焦点修复工具

```java
public final class KeydroidxDialogFocus {
...
- 弹窗期间冻结列表导航：`KeydroidxListFocusHelper.setDirectionEnabled(false)` → [06-list-focus](./06-list-focus.md)；`KeydroidxListPageFragment.isDirectionEnabled()` → [05-page-framework](./05-page-framework.md)
