# 12 · 安装统计上报（POST /install）

> 首次安装 + 版本升级时自动上报一次设备信息，服务端按 `(app, android_id)` 去重。
> 与 [10 · 反馈上报](./11-feedback.md) 共用同一台服务器、同一份配置与鉴权方式、
> 同一个 `KeydroidxFeedbackConfig`，仅接口路径与请求体不同。
...
| 设备信息位置 | 请求头（由 SDK 组装） | Body JSON |
| 鉴权 | 与 `/upload` 完全一致（由 SDK 内部完成） | **完全相同** |
...
`InstallUploader` 不重复实现签名算法，直接复用同包 `FeedbackUploader` 的静态方法，
**确保两端算法永远一致**。
...
在 `Application.onCreate` 里，紧跟 `KeydroidxFeedback.init(...)` 之后调用一行：
...
> 未初始化 `KeydroidxFeedback` 或配置无效时静默跳过，不影响反馈上报能力。
...
`KeydroidxInstall.reportOnce()` 内部用 `SharedPreferences`（`nokia_install_report`）
...
| 字段 | 来源 | 约束 |
|---|---|---|
| `app` | `KeydroidxFeedbackConfig.appName` | 1–32，`[A-Za-z0-9_-]` |
| `app_version` | `KeydroidxFeedbackConfig.appVersion` | 1–32 |
...
| 诊断日志 | tag `KeydroidxInstall` / `InstallUploader`，走 `KeydroidxLog` |
...
```
              ...
              ▼                                     ▼
   ┌────────────────────┐              ┌────────────────────┐
   │   KeydroidxFeedback     │              │   KeydroidxInstall     │
   │   submit()          │              │   reportOnce()    │
...
1. **自动触发**：上报由 `Application.onCreate` 里的 `KeydroidxInstall.reportOnce(this)` 自动触发，无需用户点击；sample 里的两个按钮仅用于反复手动测试（「清除记录并重新上报」会清 `nokia_install_report` prefs 强制重报）。
...
```bash
adb logcat KeydroidxInstall:V InstallUploader:V SampleApp:V *:S
```
