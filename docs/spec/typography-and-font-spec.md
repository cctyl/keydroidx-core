...
5. **Token 即法律**
   ...
   - 一律引用 `@dimen/nokia_font_*` Token 或 `KeydroidxFontManager.setTextSizeResource(tv, R.dimen.nokia_font_*)`。
...
| **`@dimen/nokia_font_title`** | **13sp** | 页面/弹窗标题 | 页面大标题、弹窗标题栏、软键中键主操作、播放器歌名 | `KeydroidxBaseActivity` 顶栏/中键、`KeydroidxOptionsDialog` 标题 |
| **`@dimen/nokia_font_body`** | **12sp** | **核心正文/列表项** | 桌面所有单列设置菜单（5.png）、意见反馈主 Label、歌曲名/歌单名、弹窗确认正文 | 桌面设置列表项、反馈表单项、`item_song` 主标题 |
| **`@dimen/nokia_font_small_title`** | **11sp** | 小标题/操作按钮 | 左右软键文本、表单提交按钮、分组小标题栏（Section Header）、弹窗列表选项 | `tv_soft_left` / `tv_soft_right`、提交按钮、`KeydroidxOptionsDialog` 行 |
...
| **弹窗标题栏 (Dialog Title)** | `height="26dp"` | `nokia_font_title` (13sp) | — | 背景为主题色渐变 |
| **弹窗内容行 (Dialog Row)** | `minHeight="32dp ~ 36dp"` | `nokia_font_small_title` (11sp) | — | 选中时圆角高亮 |
| **软键栏 (Softkey Bar)** | `height="22dp"` | `nokia_font_small_title` (11sp) | — | 中间标题按可用**宽度实测**自适应缩小（`fitCenterTextToWidth`：逐步 -0.5dp，下限 6dp），基准 11sp |
...
KeydroidxFontManager.setTextSizeResource(myTextView, R.dimen.nokia_font_body);
...
1. **必须用 Token，禁止裸写数字**
   ...
   - 代码：`KeydroidxFontManager.setTextSizeResource(tv, R.dimen.nokia_font_body)` ✅ / `tv.setTextSize(9f)` ❌
...
| 小数 sp | 关于页 `8.5sp` | ✅ 已收口（2026-09-06）：改 `nokia_font_caption` (9sp)（Token 体系中**没有 8sp**） |
| 关于页/选 App 页裸写数字 | `textSize="8sp"/"8.5sp"/"10sp"`、`setTextSize(tv, 9)` 等 | ✅ 已收口（2026-09-06）：全部改引用 `@dimen/nokia_font_*`；真机确认 9sp 观感可接受，**不新增 8sp 档位** |
...
>
> 关于 8sp：关于页/选 App 页曾是 8sp / 8.5sp。不上新档位，统一收口到 `nokia_font_caption` (9sp)，2026-09-06 真机（foucs）确认观感可接受。若将来确有"比 caption 更小、但非角标"的阅读型文字需求，再走**新增档位 + 同步本文件 §2/§9 与 `dimens.xml`** 的流程，不要在 XML 里裸写 8sp。
