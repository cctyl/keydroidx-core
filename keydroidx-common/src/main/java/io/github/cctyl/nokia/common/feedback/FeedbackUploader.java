 ...
 *
 * <p>请求签名、请求头与报文格式全部在本类内部完成，不对外暴露。</p>
 */
public final class FeedbackUploader {
     ...
     * 收集日志并提交反馈（全量参数）。
     *
     * @param url          服务上传地址（如 "https://your.server.com/upload"）
    ...
    public static boolean submit(String url, String secretKeyHex,
        ...
        try {
            ...
            KeydroidxLog.w(TAG, "submit error: " + t.getClass().getSimpleName() + ": " + t.getMessage());
            return false;
        }
    }
    ...
    private static boolean send(String urlStr, byte[] secretKey, String metaJson, byte[] zip) {
        ...
        try {
            ...
            // meta 采用 URL 编码避免非 ASCII / 特殊字符破坏 HTTP Header
            ...
            KeydroidxLog.w(TAG, "send failed: " + e.getClass().getSimpleName() + ": " + e.getMessage());
            return false;
        } finally {
            if (conn != null) {
                try {
