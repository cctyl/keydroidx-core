 ...
 *
 * <p>与 {@link FeedbackUploader} 共用同一台服务器、同一套鉴权方式，仅接口路径与请求体不同：</p>
 * <ul>
 *   <li>方法：POST /install</li>
 *   <li>请求头：鉴权部分与 /upload 完全一致</li>
 *   <li>Content-Type：application/json</li>
 *   <li>Body：JSON 对象（设备信息字段）</li>
 *   <li>应答：HTTP 200 表示成功；其余一律视为失败</li>
 * </ul>
 *
 * <p>签名、编码直接复用 {@link FeedbackUploader} 的 package-private 静态方法，
 * 不重复实现，确保两端算法永远一致。</p>
 *
...
public final class InstallUploader {
    ...
    public static boolean send(String url, String secretKeyHex, String json) {
        ...
        if (body.length > MAX_BODY) {
            KeydroidxLog.w(TAG, "install body too large: " + body.length);
            return false;
        }
        byte[] secretKey;
        try {
            ...
            KeydroidxLog.w(TAG, "secret key invalid: " + t.getMessage());
            return false;
        }
    ...
    private static boolean doSend(String urlStr, byte[] secretKey, byte[] body) {
        ...
        try {
            ...
            if (code != 200) {
                KeydroidxLog.w(TAG, "server response code: " + code);
            }
            return code == 200;
        } catch (Throwable t) {
            KeydroidxLog.w(TAG, "send failed: " + t.getClass().getSimpleName()
                    + ": " + t.getMessage());
            return false;
        } finally {
            if (conn != null) {
