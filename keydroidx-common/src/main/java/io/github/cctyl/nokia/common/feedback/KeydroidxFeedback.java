 ...
 *       null)); // null = 使用默认日志目录 Android/data/&lt;包名&gt;/files/log
 * </pre>
 */
public class KeydroidxFeedback {

    private static final String TAG = "KeydroidxFeedback";
    private static volatile KeydroidxFeedbackConfig sCachedConfig;
    private static final ExecutorService sExecutor = Executors.newSingleThreadExecutor();
    private static final Handler sMainHandler = new Handler(Looper.getMainLooper());

    protected KeydroidxFeedback() {
    }

    /**
     * 注册全局配置（APP 启动时调用一次）。
     */
    public static void init(KeydroidxFeedbackConfig config) {
     ...
     * 获取已注册的配置，未注册返回 null。
     */
    public static KeydroidxFeedbackConfig getConfig() {
    ...
    public static boolean isConfigured() {
        KeydroidxFeedbackConfig cfg = sCachedConfig;
        return cfg != null && cfg.isValid();
    }

    /**
     * 解析实际使用的日志目录。
     * 如果配置中指定了 logDir 则优先使用；否则使用默认目录 Android/data/<包名>/files/log
     */
    public static File resolveLogDir(Context context) {
        ...
        return context != null ? KeydroidxLog.getDefaultLogDir(context) : null;
    }
    ...
    public static void submit(Context context,
        ...
        if (cfg == null || !cfg.isValid()) {
            KeydroidxLog.w(TAG, "submit aborted: KeydroidxFeedbackConfig not initialized or invalid");
            if (callback != null) {
