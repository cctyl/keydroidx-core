 ...
 * <h3>接入示例</h3>
 * <pre>{@code
 * // Application.onCreate 中，紧跟 KeydroidxFeedback.init(...) 之后调用
 ...
 */
public final class KeydroidxInstall {

    private static final String TAG = "KeydroidxInstall";
     ...
     * 不阻塞主线程、不抛异常、不等结果。</p>
     *
     * <p>未调用 {@link KeydroidxFeedback#init(KeydroidxFeedbackConfig)} 或配置无效时静默跳过，
     * 不会影响反馈上报能力。</p>
     *
    ...
    public static void reportOnce(Context context) {
        ...
        final KeydroidxFeedbackConfig cfg = KeydroidxFeedback.getConfig();
        if (cfg == null || !cfg.isValid()) {
            KeydroidxLog.w(TAG, "reportOnce aborted: KeydroidxFeedbackConfig not initialized or invalid");
            return;
        }
        final String installUrl = cfg.resolveInstallUrl();
        if (installUrl == null || installUrl.isEmpty()) {
            KeydroidxLog.w(TAG, "reportOnce aborted: install url not resolved");
            return;
        }
    ...
    private static void doReportOnce(Context context, KeydroidxFeedbackConfig cfg, String installUrl) {
        try {
            String androidId = readAndroidId(context);
            if (androidId == null) {
                // 取不到 / 全零 → 放弃本次上报，避免产生脏数据被服务端拒
                KeydroidxLog.w(TAG, "reportOnce aborted: android_id unavailable");
                return;
            }
            ...
            if (ok) {
                ...
                prefs(context).edit()
                        ...
                        .apply();
                KeydroidxLog.d(TAG, "install report sent: " + cfg.appName + " v" + cfg.appVersion);
            } else {
                KeydroidxLog.w(TAG, "install report failed, will retry next launch");
            }
        } catch (Throwable t) {
            // 全程兜底：统计失败绝不能影响 APP 正常使用
            KeydroidxLog.w(TAG, "reportOnce error: " + t.getClass().getSimpleName()
                    + ": " + t.getMessage(), t);
        }
    }
