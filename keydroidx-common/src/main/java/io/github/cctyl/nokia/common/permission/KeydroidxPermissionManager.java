 ...
 *   <li>弹窗完全遵循 Nokia 复古按键交互规范（复用 {@link KeydroidxConfirmDialog}），绝不弹出 Material/原生触屏对话框；</li>
 *   <li>支持权限说明解释、批量申请、被永久拒绝后一键跳转系统设置页。</li>
 * </ul>
 * </p>
 */
public final class KeydroidxPermissionManager {

    private static final String TAG = "KeydroidxPermissionManager";
    ...
    private KeydroidxPermissionManager() {}
    ...
    public static void request(@NonNull Activity activity,
        ...
        XXPermissions.with(activity)
                ...
                .request(new OnPermissionCallback() {
                    ...
                    @Override
                    public void onDenied(@NonNull List<String> denied, boolean quick) {
                        KeydroidxLog.w(TAG, "Permissions denied: " + denied + ", quick=" + quick);
                        callback.onDenied(denied, quick);
                    }
                });
    }
    ...
    public static void requestWithNokiaDialog(@NonNull Activity activity,
        ...
        KeydroidxConfirmDialog dialog = new KeydroidxConfirmDialog(activity, title, message);
    ...
    public static void requestCorePermissions(@NonNull Activity activity,
        ...
        if (needed.isEmpty()) {
            KeydroidxLog.i(TAG, "核心权限全集已就绪，无需申请");
            callback.onGranted(all, true);
            return;
        }
        KeydroidxLog.i(TAG, "启动核心权限批量申请，缺失: " + needed);
