...
import io.github.cctyl.nokia.common.ui.page.KeydroidxPage;
 ...
 * 标题栏、三段式软键条、Touch Mode 首键防吞噬、页面契约（{@link KeydroidxPage}）与防抖物理按键分发。
 * <p>
 * 零宿主业务依赖，桌面 Launcher 与独立 App（key-core）均继承此基类。
 */
public abstract class KeydroidxBaseActivity extends AppCompatActivity implements KeydroidxPageHost, KeyResolver {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // 软键默认点击事件 (触屏回退)
        if (tvSoftLeft != null) {
            tvSoftLeft.setOnClickListener(v -> onAction(KeydroidxKeyAction.SOFT_LEFT));
        }
        if (tvSoftCenter != null) {
            tvSoftCenter.setOnClickListener(v -> onAction(KeydroidxKeyAction.SELECT));
        }
        if (tvSoftRight != null) {
            tvSoftRight.setOnClickListener(v -> onAction(KeydroidxKeyAction.SOFT_RIGHT));
        }

        // 主动触发一次主题与字体应用
        KeydroidxTheme.ThemeDef currentTheme = KeydroidxUi.getTheme(this);
    ...
    public void setTitleIcon(CharSequence iconCode) {
        if (tvTitleIcon != null) {
            if (iconCode != null && iconCode.length() > 0) {
                KeydroidxIcons.setIcon(tvTitleIcon, iconCode.toString());
    ...
    public void setSignalIcon(CharSequence iconCode) {
        if (tvSignalIcon != null && iconCode != null) {
            KeydroidxIcons.setIcon(tvSignalIcon, iconCode.toString());
        }
    }
    ...
    private void updateBatteryInfo(Intent intent) {
        ...
        if (tvBatteryIcon != null && percent >= 0) {
            KeydroidxBatteryDrawable drawable = null;
            if (tvBatteryIcon.getDrawable() instanceof KeydroidxBatteryDrawable) {
                drawable = (KeydroidxBatteryDrawable) tvBatteryIcon.getDrawable();
            }
            if (drawable == null) {
                drawable = new KeydroidxBatteryDrawable(this);
     ...
     * 应用主题到骨架组件。
     */
    public void applyTheme(@NonNull KeydroidxTheme.ThemeDef theme) {
        if (titleBar != null) {
     ...
     * key-core 子类可重写以委托 {@code KeydroidxClient}。
     */
    @Override
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            ...
            KeydroidxLog.d(TAG, "dispatchKeyEvent keyCode=" + keyCode
    ...
    protected KeydroidxPage getCurrentPage() {
        if (this instanceof KeydroidxPage) {
            return (KeydroidxPage) this;
        }
        androidx.fragment.app.FragmentManager fm = getSupportFragmentManager();
        androidx.fragment.app.Fragment primaryNav = fm.getPrimaryNavigationFragment();
        if (primaryNav instanceof KeydroidxPage && primaryNav.isAdded() && !primaryNav.isHidden()) {
            return (KeydroidxPage) primaryNav;
        }
        ...
        for (int i = fragments.size() - 1; i >= 0; i--) {
            ...
            if (f != null && f.isAdded() && !f.isHidden() && f instanceof KeydroidxPage) {
                return (KeydroidxPage) f;
            }
        }
        return null;
    }
    ...
    public void refreshPageBar() {
        KeydroidxPage page = getCurrentPage();
        if (page != null) {
    ...
    protected boolean onAction(int action) {
        ...
        if (page != null) {
            ...
            switch (action) {
                case KeydroidxKeyAction.UP:
                case KeydroidxKeyAction.DOWN:
                case KeydroidxKeyAction.LEFT:
                case KeydroidxKeyAction.RIGHT:
                    handled = page.onDirection(action);
                    break;
                case KeydroidxKeyAction.SELECT:
                    handled = page.onSelect();
                    break;
                case KeydroidxKeyAction.SOFT_LEFT:
                    handled = page.onSoftLeft();
                    break;
                case KeydroidxKeyAction.SOFT_RIGHT:
        ...
        if (action == KeydroidxKeyAction.SOFT_RIGHT) {
            finish();
            return true;
        }
        return false;
