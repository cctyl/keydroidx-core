 ...
 * 规范见 {@code docs/spec/typography-and-font-spec.md}。
 * <p>
 ...
 * <p>
 * 本类同时承载原 Launcher 本地 {@code KeydroidxFontManager} 的全部能力（自定义外部字体导入、
 * {@code textSize}、{@code applyFontToViewHierarchy}、{@code FontItem} 等），以便桌面与衍生 App
 * 共用同一份实现，不再有第二套静态缩放字段。
 */
public class KeydroidxFontManager {

    private static final String TAG = "KeydroidxFontManager";
     ...
     * 原 Launcher 本地 KeydroidxFontManager.setUserFontScale 的同义入口，保留以减少迁移面。
     */
    ...
    public static Typeface loadTypeface(Context context, String fontId) {
        ...
        try {
            ...
            KeydroidxLog.e(TAG, "加载字体失败: " + fontId, t);
        }
    ...
    public static String importFontFromUri(Context context, Uri uri) {
        ...
        try {
            ...
            KeydroidxLog.e(TAG, "导入字体文件失败: " + uri, t);
            return null;
        }
    }
    ...
    private static void applyTypefaceToSingleView(TextView tv) {
        ...
        Typeface iconTf = KeydroidxIcons.getTypeface(tv.getContext());
