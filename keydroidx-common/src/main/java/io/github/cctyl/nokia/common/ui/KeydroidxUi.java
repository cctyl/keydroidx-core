 ...
 * 能力的统一入口，并带降级兜底：</p>
 * <ul>
 *   <li>若宿主（如 {@code KeydroidxBaseActivity}）实现了相应接口，直接复用宿主实现
 *       （跨进程按键同步 / 主题缓存）；</li>
...
public final class KeydroidxUi {
     ...
     * （由 {@link io.github.cctyl.nokia.common.ui.KeydroidxTheme#setThemeProvider} 注册），
     * 未注册或查询失败时回退到默认 Classic Blue 主题。
     */
    @NonNull
    public static KeydroidxTheme.ThemeDef getTheme(@Nullable Context context) {
        // 优先复用全局 ThemeProvider（独立 App 的 KeydroidxClient / 桌面的 LauncherThemeProvider）
        try {
