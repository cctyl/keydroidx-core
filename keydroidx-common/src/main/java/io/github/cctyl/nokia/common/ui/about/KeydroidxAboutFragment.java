 ...
 */
public class KeydroidxAboutFragment extends KeydroidxScrollPageFragment {

    protected static final String ARG_CONFIG = "arg_about_config";

    private KeydroidxAboutConfig config;
    ...
    public static KeydroidxAboutFragment newInstance(@Nullable KeydroidxAboutConfig config) {
        KeydroidxAboutFragment fragment = new KeydroidxAboutFragment();
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null && getArguments().containsKey(ARG_CONFIG)) {
            config = (KeydroidxAboutConfig) getArguments().getSerializable(ARG_CONFIG);
        }
        if (config == null && getContext() != null) {
            config = KeydroidxAboutConfig.createDefault(getContext());
        }
    }
    ...
    private void initViews(@NonNull View root) {
        ...
        // 3.5 额外自定义链接
        for (KeydroidxAboutConfig.LinkItem link : config.getExtraLinks()) {
        ...
        if (!TextUtils.isEmpty(config.getAuthor())) {
            KeydroidxIcons.setIcon(iconAuthor, KeydroidxIcons.PERSON);
        ...
        // 应用点阵字体与缩放
        KeydroidxLog.d("KeydroidxAboutFragment", "about render fontScale=" + KeydroidxFontManager.getFontScale()
                + " fontId=" + KeydroidxFontManager.getCurrentFontId()
                + " process=" + android.os.Process.myPid());
        KeydroidxFontManager.applyToViewTree(root);
    ...
    private void checkUpdate() {
        ...
        KeydroidxUpdateConfig updateConfig = new KeydroidxUpdateConfig(config.getRepoUrl());
    ...
    private void openMoreApps() {
        ...
        if (getActivity() instanceof KeydroidxPageHost) {
            ((KeydroidxPageHost) getActivity()).openFragment(KeydroidxMoreAppsFragment.newInstance(apps));
        }
    }
    ...
    private void updateFocusHighlight() {
        ...
        for (int i = 0; i < interactiveItems.size(); i++) {
            ...
            if (i == focusedIndex) {
                item.cardView.setBackground(KeydroidxTheme.createSelectionDrawable(ctx, dp(4)));
            } else {
    ...
    private void toggleDetailedLog() {
        if (getContext() == null) return;
        Context ctx = getContext();
        boolean newState = !KeydroidxLog.isDetailedLogEnabled(ctx);
        KeydroidxLog.setDetailedLogEnabled(ctx, newState);
    ...
    private void showUrlOptions(InteractiveItem item) {
        ...
        KeydroidxOptionsDialog dialog = new KeydroidxOptionsDialog(getContext(), item.title)
                .addItem(1, "在浏览器中打开")
                .addItem(2, "复制链接到剪贴板")
                .addItem(3, "返回")
    ...
    public boolean onDirection(int direction) {
        if (direction == KeydroidxKeyAction.UP) {
            if (focusedIndex > 0) {
                focusedIndex--;
                updateFocusHighlight();
        ...
        } else if (direction == KeydroidxKeyAction.DOWN) {
