 ...
 * <p>
 * 继承 {@link KeydroidxListPageFragment} 复用循环焦点导航 + 高亮 + 滚动跟随。
 */
public class KeydroidxMoreAppsFragment extends KeydroidxListPageFragment {
    ...
    public static KeydroidxMoreAppsFragment newInstance(@Nullable List<KeydroidXApps.App> apps) {
        KeydroidxMoreAppsFragment f = new KeydroidxMoreAppsFragment();
    ...
    public boolean onSoftRight() {
        if (getActivity() instanceof KeydroidxPageHost) {
            ((KeydroidxPageHost) getActivity()).exitCurrent();
            return true;
        }
        return false;
    }
    ...
    private void showUrlOptions(String title, String url) {
        ...
        new KeydroidxOptionsDialog(getContext(), title)
                .addItem(1, "在浏览器中打开")
                .addItem(2, "复制链接到剪贴板")
                .addItem(3, "返回")
