...
public class KeydroidxOptionsDialog extends Dialog {
    ...
    public KeydroidxOptionsDialog(@NonNull Context context) {
        this(context, "选项");
    }
    ...
    public KeydroidxOptionsDialog addItem(int id, CharSequence title) {
        this.items.add(new OptionItem(id, title));
        return this;
    }

    public KeydroidxOptionsDialog addItem(int id, CharSequence title, Drawable icon) {
        this.items.add(new OptionItem(id, title, icon));
        return this;
    }

    public KeydroidxOptionsDialog addItem(CharSequence title) {
    ...
    private void initViews() {
        ...
        int rowHeight = KeydroidxDimens.dp(context.getResources(), 32);
        int iconSize = KeydroidxDimens.dp(context.getResources(), 18);
        int iconMargin = KeydroidxDimens.dp(context.getResources(), 8);
        int paddingH = KeydroidxDimens.dp(context.getResources(), 12);
    ...
    private void updateSelection() {
        ...
        for (int i = 0; i < itemViews.size(); i++) {
            ...
            if (isSelected) {
                ...
                gd.setCornerRadius(KeydroidxDimens.dp(getContext().getResources(), 4));
    ...
    public boolean dispatchKeyEvent(@NonNull KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            ...
            if (action == KeydroidxKeyAction.UP) {
                if (selectedIndex > 0) {
                    selectedIndex--;
                    updateSelection();
                }
                return true;
            } else if (action == KeydroidxKeyAction.DOWN) {
                if (selectedIndex < items.size() - 1) {
                    selectedIndex++;
                    updateSelection();
                }
                return true;
            } else if (action == KeydroidxKeyAction.SELECT || action == KeydroidxKeyAction.SOFT_LEFT) {
                confirmSelection();
                return true;
