...
public class KeydroidxDashedLineDrawable extends Drawable {
    ...
    public KeydroidxDashedLineDrawable(Resources res, int color) {
     ...
     */
    public KeydroidxDashedLineDrawable(Resources res, int color, float dotSizeDp, float gapSizeDp) {
        mPaint.setColor(color);
        mPaint.setStyle(Paint.Style.FILL);  // FILL 无抗锯齿羽化，drawRect 全版本硬件加速正常
        mPaint.setAntiAlias(false);         // 点阵无需抗锯齿
        mDotPx = Math.max(1f, KeydroidxDimens.dpF(res, dotSizeDp));
        mGapPx = Math.max(1f, KeydroidxDimens.dpF(res, gapSizeDp));
    }
