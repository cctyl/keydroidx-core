 ...
 * </ol>
 */
public abstract class KeydroidxPageFragment extends Fragment implements KeydroidxPage {
    ...
    public void notifyHostRefresh() {
        ...
        if (activity instanceof KeydroidxPageHost) {
            ((KeydroidxPageHost) activity).refreshPageBar();
        }
    }
    ...
    public void smoothScrollToVisible(@Nullable ScrollView scroll, @Nullable View target) {
        ...
        scroll.post(() -> {
            ...
            KeydroidxListFocusHelper.smoothScrollToVisible(scroll, target);
        });
    }

    // ---- KeydroidxPage 默认契约实现（子类按需重写） ----
