...
/**
 * 检查更新门面（纯逻辑、无 UI，风格对齐 {@code KeydroidxFeedback}）。
 *
 * <p>通过 GitHub Releases API 查询远端最新版本号，与宿主当前版本号对比；
 * GitHub 网络不畅导致检查失败时，宿主可用
 * {@link KeydroidxUpdateConfig#getFallbackUrl()}（默认百度网盘）引导用户手动下载。</p>
 *
 * <h3>接入示例（无 UI）</h3>
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>想要内置复古弹窗交互，直接用 {@link KeydroidxUpdateDialog#checkAndShow(Context, KeydroidxUpdateConfig)}。</p>
 *
 * <p>宿主需声明 {@code android.permission.INTERNET}。</p>
 */
public final class KeydroidxUpdateChecker {

    private static final String TAG = "KeydroidxUpdateChecker";
    ...
    private static Handler mainHandler() {
        if (sMainHandler == null) {
            synchronized (KeydroidxUpdateChecker.class) {
                if (sMainHandler == null) {
    ...
    private KeydroidxUpdateChecker() {
    }

    /** 异步检查结果回调（保证在主线程派发） */
    public interface Callback {
        void onResult(KeydroidxUpdateResult result);
    }
     ...
     * {@link KeydroidxUpdateResult.Status#FAILED}，绝不抛出。</p>
     *
     ...
     */
    public static void check(Context context, KeydroidxUpdateConfig config, Callback callback) {
        final String currentVersion = config != null
                ? config.resolveCurrentVersion(context) : "0";
        if (config == null || !config.isValid()) {
            KeydroidxLog.w(TAG, "check aborted: invalid config (repoUrl unreadable)");
            if (callback != null) {
                callback.onResult(KeydroidxUpdateResult.failed("配置无效：无法从仓库地址解析出 owner/repo", currentVersion));
            }
            return;
        }
        sExecutor.execute(new Runnable() {
            @Override
            public void run() {
                final KeydroidxUpdateResult result = doCheck(context != null
    ...
    /** 后台线程执行的真实检查逻辑 */
    private static KeydroidxUpdateResult doCheck(Context appContext,
                                             KeydroidxUpdateConfig config, String currentVersion) {
        try {
            String repoPath = config.resolveRepoPath();
            String body;
            if (config.isIncludePreRelease()) {
                // releases 列表按创建时间倒序，取第一个（含 pre-release）
                body = httpGet(KeydroidxUpdateConfig.GITHUB_API_BASE + repoPath + "/releases?per_page=5",
                        config.getTimeoutMs());
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    KeydroidxUpdateInfo info = parseRelease(rel, config);
                    if (info == null) {
                        continue;
                    }
                    return buildResult(info, currentVersion);
                }
                return KeydroidxUpdateResult.failed("仓库暂无可用 Release", currentVersion);
            } else {
                body = httpGet(KeydroidxUpdateConfig.GITHUB_API_BASE + repoPath + "/releases/latest",
                        config.getTimeoutMs());
            ...
            KeydroidxLog.w(TAG, "check failed: " + t.getClass().getSimpleName()
                    + ": " + t.getMessage(), t);
            return KeydroidxUpdateResult.failed("GitHub 访问失败：" + briefReason(t), currentVersion);
        }
    }

    private static KeydroidxUpdateResult buildResult(KeydroidxUpdateInfo info, String currentVersion) {
        if (info.isNewerThan(currentVersion)) {
            return KeydroidxUpdateResult.available(info, currentVersion);
        }
        return KeydroidxUpdateResult.upToDate(info, currentVersion);
    }
     ...
     */
    static KeydroidxUpdateInfo parseRelease(JSONObject rel, KeydroidxUpdateConfig config) {
        String tag = rel.optString("tag_name", "").trim();
        if (tag.isEmpty()) {
            return null;
        }
        String version = KeydroidxUpdateConfig.normalizeVersion(tag);
        ...
        return new KeydroidxUpdateInfo(version, tag,
