 ...
 *
 * <p>纯逻辑检查请直接用 {@link KeydroidxUpdateChecker#check}；宿主不想自建界面时，
 ...
 *       {@link KeydroidxUpdateConfig#getFallbackUrl()}（默认百度网盘）。</li>
 * </ul>
 *
 * <p>注意：context 必须是 Activity（弹窗需要窗口 token）。</p>
 */
public final class KeydroidxUpdateDialog {

    private static final String TAG = "KeydroidxUpdateDialog";
    /** 更新说明在弹窗里的最大展示长度（超出截断加省略号） */
    private static final int MAX_CHANGELOG = 160;

    private KeydroidxUpdateDialog() {
    }
    ...
    public static void checkAndShow(@NonNull final Context context,
                                    @NonNull final KeydroidxUpdateConfig config) {
        KeydroidxUpdateChecker.check(context, config, new KeydroidxUpdateChecker.Callback() {
            @Override
    ...
    public static void showResult(@NonNull Context context,
                                  @Nullable KeydroidxUpdateResult result,
                                  @Nullable String fallbackUrl) {
        if (result == null) {
            return;
        }
        String fallback = (fallbackUrl == null || fallbackUrl.trim().isEmpty())
                ? KeydroidxUpdateConfig.DEFAULT_FALLBACK_URL : fallbackUrl;
    ...
    private static void showUpdateAvailable(Context context, KeydroidxUpdateResult result) {
        KeydroidxUpdateInfo info = result.info;
    ...
    private static void showUpToDate(Context context, KeydroidxUpdateResult result) {
        new KeydroidxConfirmDialog(context, "已是最新版本",
    ...
    private static void showFailed(Context context, String fallbackUrl) {
        new KeydroidxConfirmDialog(context, "检查更新失败",
                "无法连接 GitHub 获取版本信息。\n可前往网盘手动下载最新版本。")
    ...
    private static void openUrl(Context context, String url) {
        if (url == null || url.trim().isEmpty()) {
            KeydroidxLog.w(TAG, "openUrl aborted: empty url");
            return;
        }
        try {
            ...
            KeydroidxLog.w(TAG, "openUrl failed: " + url, t);
        }
    }
